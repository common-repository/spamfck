<?php
	/*
	Plugin Name: Antispam Revolution
...
// CREATE Antispam Revolution STATISTICS
add_option( 'antispamrevolution_stats', '0', '', 'yes' );

// THIRD WAY OF PROTECTION - COMPARE TIME USER SPENT ON THE PAGE AND TIME WHEN HE SEND A COMMENT
setcookie("antispamrevolution-usertime-start", time());

 // FIRST WAY OF PROTECTION - ADD HIDDEN FIELDS / VALIDATION CHECKBOX TO FORMS
 //1. Add a new form elements to registration form
    add_action('register_form','antispamrevolution_register_form');
    function antispamrevolution_register_form (){
 ...
 $description = sanitize_text_field( $description );
   ...
   $robot_checkbox_validation = get_option('antispamrevolution');
 if ($robot_checkbox_validation == "yes"){
   ...
   <p>
    ...
    add_filter('registration_errors', 'antispamrevolution_registration_errors', 10, 3);
    function antispamrevolution_registration_errors ($errors, $sanitized_user_login, $user_email) {

// THIRD WAY OF PROTECTION - COMPARE TIME USER SPENT ON THE PAGE AND TIME WHEN HE SEND A COMMENT
$usertime_start = $_COOKIE['antispamrevolution-usertime-start'];
$actualtime = time();
...
// --THIRD WAY OF PROTECTION - COMPARE TIME USER SPENT ON THE PAGE AND TIME WHEN HE SEND A COMMENT
 ...
 $antispamrevolutionstats = get_option('antispamrevolution_stats');   //Spam statistics
 $antispamrevolutionstats_update = $antispamrevolutionstats+1;        //Spam statistics update
    ...
    if($usertimecompare < 5)     //5 seconds        // THIRD WAY OF PROTECTION - COMPARE TIME USER SPENT ON THE PAGE AND TIME WHEN HE SEND A COMMENT
            ...
            update_option( 'antispamrevolution_stats', $antispamrevolutionstats_update, 'yes' );     //Spam statistics update
    if ( !empty( $_POST['web'] ) )
...
add_action( 'admin_menu', 'antispamrevolution_menu' );

// Create WordPress admin menu
if( !function_exists("antispamrevolution_menu") )
{
function antispamrevolution_menu(){

  $page_title = 'Antispam Revolution';
  $menu_title = 'Antispam Revolution';
  $capability = 'manage_options';
  $menu_slug  = 'antispamrevolution';
  $function   = 'antispamrevolution_page';
  ...
  // Call update_antispamrevolution function to update database
  add_action( 'admin_init', 'update_antispamrevolution' );

}
}

// Create function to register plugin settings in the database
if( !function_exists("update_antispamrevolution") )
{
function update_antispamrevolution() {
  register_setting( 'antispamrevolution-settings', 'antispamrevolution' );     
}
}

// Create WordPress plugin page
if( !function_exists("antispamrevolution_page") )
{
function antispamrevolution_page(){
?>
  <h1>Antispam Revolution settings page</h1>
  <h2>This plugin:</h2>
  ...
  <form method="post" action="options.php">
    <?php settings_fields( 'antispamrevolution-settings' ); ?>
    <?php do_settings_sections( 'antispamrevolution-settings' ); ?>
    <table class="form-table">
      <tr valign="top">
      <th scope="row">Allow 'I am robot' checkbox validation:</th>
      <td>
      <select name='antispamrevolution'>
      <?php $selectedoption = get_option('antispamrevolution'); 
  ...
  <div>
  This page has been protected <strong><?php echo $antispamrevolutionstats = get_option('antispamrevolution_stats');?></strong> times.
  </div>
...
if( !function_exists("antispamrevolution") )
{
  function antispamrevolution($content)
  {
    $extra_info = get_option('antispamrevolution');
    return $content;
  }
}

// Apply the antispamrevolution function on our content  
add_filter('the_content', 'antispamrevolution');
...
fwrite($htaccess, "\n"); 
fwrite($htaccess, "\n"); 
fwrite($htaccess, "# BEGIN Antispam Revolution");
fwrite($htaccess, "\n"); 
...
fwrite($htaccess, "\n");
fwrite($htaccess, "# END Antispam Revolution");
fclose($htaccess);
}
